...
export const PlaceholdersAndVanishInput: AceternityComponent<{
  placeholders: string[];
  onChange: React.ChangeEventHandler<HTMLInputElement>;
  onSubmit: React.FormEventHandler<HTMLFormElement>;
}> = ({ placeholders, onChange, onSubmit }) => {
  const [currentPlaceholder, setCurrentPlaceholder] = useState(0);

  const intervalRef = useRef<NodeJS.Timeout | null>(null);

  const startAnimation = useCallback(() => {
    intervalRef.current = setInterval(() => setCurrentPlaceholder((prev) => (prev + 1) % placeholders.length), 3000);
  }, [placeholders.length]);
  ...
  const getOffset = () => (Math.random() > 0.5 ? 1 : -1);
