...
export const TracingBeam: AceternityComponent<{
  stiffness?: number;
  damping?: number;
}> = ({ children, className, stiffness = 500, damping = 90 }) => {
  ...
  const y1 = useSpring(useTransform(scrollYProgress, [0, 0.8], [50, svgHeight]), { stiffness, damping });
  const y2 = useSpring(useTransform(scrollYProgress, [0, 1], [50, svgHeight - 200]), { stiffness, damping });
      ...
      <div className="absolute -left-4 top-3 md:-left-20">
        ...
        >
          <motion.div
            transition={{ duration: 0.2, delay: 0.5 }}
            animate={{
              backgroundColor: scrollYProgress.get() > 0 ? 'rgb(255 255 255)' : 'var(--emerald-500)',
              borderColor: scrollYProgress.get() > 0 ? 'rgb(255 255 255)' : 'var(--emerald-600)'
            }}
